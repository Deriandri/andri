...
wget -q -O /usr/bin/helium "https://raw.githubusercontent.com/Deriandri/andri/main/menu/helium.sh" && chmod +x /usr/bin/helium && helium
fi
fi
}
function add-host(){
...
if [[ $dobot == "1" ]]; then
clear
wget https://raw.githubusercontent.com/Deriandri/andri/main/dobot/install.sh &> /dev/null
chmod +x install.sh
bash install.sh
rm -rf install.sh
fi
